...
export default function TutorialPage() {
  ...
  const fetchData = async () => {
    const { data: res } = await supabase.from('tutorials').select('*').order('created_at', { ascending: false }).limit(100);
    if (res) setData(res);
    setLoading(false);
  };
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Are you sure you want to delete this?')) {
      await deleteRecord('tutorials', id, '/admin/dashboard/tutorials');
      fetchData();
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link 
          href="/admin/dashboard/tutorials/new" 
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <DataTable 
          ...
          editLinkPrefix="/admin/dashboard/tutorials"
        />
      </div>
    </div>
  );

...
export default function EditPresidentSpeechPage({ params }: { params: Promise<{ id: string }> }) {
  const resolvedParams = use(params);
  
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Edit President Speech</h1>
...
function DataFetcher({ id }: { id: string }) {
  const [data, setData] = useState<PresidentSpeech | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    async function loadData() {
      const { data: res } = await supabase.from('president_speech').select('*').eq('id', id).single();
      if (res) setData(res);
      setLoading(false);
    }
    loadData();
  ...
  return <SpeechForm initialData={data} />;
}

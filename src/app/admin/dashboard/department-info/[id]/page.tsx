...
export default function EditDepartmentInfoPage({ params }: { params: Promise<{ id: string }> }) {
  const resolvedParams = use(params);
  
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Edit Department Info</h1>
...
function DataFetcher({ id }: { id: string }) {
  const [data, setData] = useState<DepartmentInfo | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    async function loadData() {
      const { data: res } = await supabase.from('department_info').select('*').eq('id', id).single();
      if (res) setData(res);
      setLoading(false);
    }
    loadData();
  ...
  return <DepartmentInfoForm initialData={data} />;
}

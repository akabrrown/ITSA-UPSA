...
export default function EditDepartmentAuthorityPage({ params }: { params: Promise<{ id: string }> }) {
...
function DataFetcher({ id }: { id: string }) {
  const [data, setData] = useState<DepartmentAuthority | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    async function loadData() {
      const { data: res } = await supabase.from('department_authorities').select('*').eq('id', id).single();
      if (res) setData(res);
      setLoading(false);
    }
    loadData();
  ...
  return <AuthorityForm initialData={data} />;
}
